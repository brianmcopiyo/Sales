...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-semibold text-primary tracking-tight">Import Devices</h1>
                <p class="text-sm font-medium text-themeMuted mt-1">Upload a CSV or Excel file to add multiple devices at once.
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <ul class="text-sm text-themeBody space-y-1 list-disc list-inside">
                <li><strong>imei</strong> — 15 digits, unique per device</li>
                <li><strong>product_sku</strong> or <strong>product_name</strong> — must match an active product</li>
                <li><strong>notes</strong> — optional</li>
            </ul>
            <p class="mt-3 text-sm text-themeMuted">Empty rows are skipped. Devices are created as &quot;Available&quot; and
                assigned to your branch.</p>
        </div>

        @if (isset($products) && $products->isNotEmpty())
            <div
                class="bg-themeCard rounded-2xl border border-themeBorder p-4 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                <h2 class="text-sm font-semibold text-themeHeading mb-2">Active product SKUs you can use</h2>
                <p class="text-sm text-themeMuted mb-3">Use either <strong>product_sku</strong> or
                    <strong>product_name</strong> in your file.
                </p>
                <div class="flex flex-wrap gap-2">
                    @foreach ($products as $p)
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-lg bg-themeHover text-themeBody text-sm font-medium">{{ $p->sku }}</span>
                    @endforeach
                </div>
            </div>
        @endif
