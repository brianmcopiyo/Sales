...
@section('title', 'Sales Stats — ' . $product->name)
    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm font-medium text-themeMuted mt-1">{{ $product->name }} @if($product->sku)<span class="text-themeBody">({{ $product->sku }})</span>@endif</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div class="bg-themeCard rounded-2xl border border-themeBorder p-5 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                <div class="text-xs font-medium text-themeMuted mb-1">Units sold</div>
                <div class="text-2xl font-semibold text-primary">{{ $stats['total_quantity'] }}</div>
            </div>
            <div class="bg-themeCard rounded-2xl border border-themeBorder p-5 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                <div class="text-xs font-medium text-themeMuted mb-1">Sales (orders)</div>
                <div class="text-2xl font-semibold text-emerald-600">{{ $stats['sales_count'] }}</div>
            </div>
            <div class="bg-themeCard rounded-2xl border border-themeBorder p-5 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
        ...
        <div class="bg-themeCard rounded-2xl border border-themeBorder overflow-hidden shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            <div class="px-6 py-4 border-b border-themeBorder">
                <h2 class="text-lg font-semibold text-primary tracking-tight">Sale items (this product)</h2>
            </div>
                        ...
                        @forelse($items as $item)
                            <tr class="hover:bg-themeInput/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-themeHeading">
                                    <a href="{{ route('sales.show', $item->sale) }}" class="text-primary hover:underline">{{ $item->sale?->sale_number ?? '—' }}</a>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    @if ($item->sale)
                                        <a href="{{ route('sales.show', $item->sale) }}" class="text-primary hover:underline">View sale</a>
                                    @endif
                                </td>
                            </tr>
                        @empty
