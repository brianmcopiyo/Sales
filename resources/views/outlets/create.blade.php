...
@section('content')
    <div class="w-full">
        <div class="mb-6 flex justify-between items-center">
            <h1 class="text-3xl font-semibold text-primary tracking-tight">Create Outlet</h1>
        ...
        <div class="bg-themeCard rounded-2xl border border-themeBorder p-6 shadow-sm">
            <form method="POST" action="{{ route('outlets.store') }}" class="space-y-6" x-data="{ branchSelected: {{ json_encode((bool) old('branch_id')) }} }">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="address" name="address" rows="2"
                            class="w-full px-4 py-2.5 border border-themeBorder rounded-xl text-themeHeading focus:ring-2 focus:ring-primary/20 focus:border-primary">{{ old('address') }}</textarea>
                    </div>
                    <div>
                        <label for="lat" class="block text-sm font-medium text-themeBody mb-1">Latitude</label>
                        <input type="number" id="lat" name="lat" value="{{ old('lat') }}" step="any" placeholder="e.g. -6.1629"
                            class="w-full px-4 py-2.5 border border-themeBorder rounded-xl text-themeHeading focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                    <div>
                        <label for="lng" class="block text-sm font-medium text-themeBody mb-1">Longitude</label>
                        <input type="number" id="lng" name="lng" value="{{ old('lng') }}" step="any" placeholder="e.g. 35.7516"
                            class="w-full px-4 py-2.5 border border-themeBorder rounded-xl text-themeHeading focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2 border-t border-themeBorder pt-4" x-data="{ geoType: '{{ old('geo_fence_type', '') }}' }">
                        ...
                        <div class="flex flex-wrap gap-4 items-center mb-4">
                            ...
                            <template x-if="geoType === 'radius'">
                                <span class="flex items-center gap-2">
                                    <input type="number" name="geo_fence_radius_metres" value="{{ old('geo_fence_radius_metres', 100) }}" min="10" max="5000" step="10" placeholder="m"
                                        class="w-24 px-3 py-2 border border-themeBorder rounded-xl text-themeHeading focus:ring-2 focus:ring-primary/20">
                                    <span class="text-sm text-themeMuted">metres from outlet</span>
                                </span>
                            </template>
                            <label class="inline-flex items-center">
                        ...
                        <div x-show="geoType === 'polygon'" x-cloak class="mt-2">
                            ...
                            <textarea id="geo_fence_polygon" name="geo_fence_polygon" rows="4" placeholder='[{"lat":-6.16,"lng":35.75},{"lat":-6.17,"lng":35.75},{"lat":-6.17,"lng":35.76},{"lat":-6.16,"lng":35.76}]'
                                class="w-full px-4 py-2.5 border border-themeBorder rounded-xl text-themeHeading font-mono text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">{{ old('geo_fence_polygon', '') }}</textarea>
                            <p class="mt-1 text-xs text-themeMuted">Array of {"lat": number, "lng": number}. First and last point may be the same to close the shape.</p>
