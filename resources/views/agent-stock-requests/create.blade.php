...
@section('title', 'Request stock from your branch')

@section('content')
    <div class="w-full space-y-6">
        @include('partials.back-button', [
            'href' => route('agent-stock-requests.index'),
            'label' => 'Back to Agent Stock Requests',
        ])
        <div>
            <h1 class="text-3xl font-semibold text-primary tracking-tight">Request stock from your branch</h1>
            <p class="text-sm font-medium text-themeMuted mt-1">Request additional stock from your branch when you're running low. Your branch can approve, partially approve, or reject.</p>
        </div>
        ...
        <div class="bg-themeCard rounded-2xl border border-themeBorder p-6 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            <form method="POST" action="{{ route('agent-stock-requests.store') }}" class="space-y-6">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-themeBody mb-2">Your branch</label>
                        <div class="w-full px-4 py-2.5 border border-themeBorder rounded-xl bg-themeInput font-medium text-themeBody">
                            {{ $branch->name ?? auth()->user()->branch->name ?? '—' }}
                        </div>
                    </div>
                    ...
                    <div>
                        <label for="quantity_requested" class="block text-sm font-medium text-themeBody mb-2">Quantity requested *</label>
