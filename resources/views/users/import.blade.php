...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-semibold text-primary tracking-tight">Import Users</h1>
                <p class="text-sm font-medium text-themeMuted mt-1">Upload a CSV or Excel file to add multiple users at once.
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <ul class="text-sm text-themeBody space-y-1 list-disc list-inside">
                ...
                <li><strong>role</strong> — Role slug or name (e.g. staff, head_branch_manager, admin). Omit for staff.</li>
            ...
            <p class="mt-3 text-sm text-themeMuted">Empty rows are skipped. A random password is generated for each user.</p>
        </div>
        ...
        @if (isset($roles) && $roles->isNotEmpty())
            <div
                class="bg-themeCard rounded-2xl border border-themeBorder p-4 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                <h2 class="text-sm font-semibold text-themeHeading mb-2">Role slugs you can use</h2>
                <div class="flex flex-wrap gap-2">
                    @foreach ($roles as $r)
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-lg bg-themeHover text-themeBody text-sm font-medium">{{ $r->slug }}</span>
                    @endforeach
                </div>
            </div>
        @endif
