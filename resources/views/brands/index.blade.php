...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <h1 class="text-3xl font-semibold text-primary tracking-tight">Brands</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div
                ...
                <div class="text-sm font-medium text-themeMuted mb-1">Total Brands</div>
        ...
        <div
            ...
            <form method="GET" action="{{ route('brands.index') }}" class="flex flex-wrap gap-4 items-end">
        ...
        <div
            ...
            <div class="md:hidden divide-y divide-themeBorder">
                @forelse($brands as $brand)
                    <a href="{{ auth()->user()?->hasPermission('brands.view') ? route('brands.show', $brand) : '#' }}"
                        class="block px-4 py-4 hover:bg-themeInput/50 transition-colors {{ !auth()->user()?->hasPermission('brands.view') ? 'pointer-events-none' : '' }}">
                        <div class="flex justify-between items-start gap-3">
                            <div class="min-w-0 flex-1">
                                <div class="text-sm font-semibold text-primary">{{ $brand->name }}</div>
                                <div class="text-xs text-themeBody mt-0.5 truncate">{{ Str::limit($brand->description ?? '-', 50) }}</div>
                                <div class="text-xs text-themeMuted mt-1">{{ $brand->products_count }} products</div>
                            </div>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium flex-shrink-0 {{ $brand->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-themeHover text-themeBody' }}">{{ $brand->is_active ? 'Active' : 'Inactive' }}</span>
                        </div>
                    </a>
                @empty
                    <div class="px-4 py-12 text-center text-themeMuted font-medium">No brands found.</div>
                @endforelse
            </div>
            {{-- Desktop: table --}}
            <div class="hidden md:block overflow-x-auto">
                <table class="min-w-full divide-y divide-themeBorder">
                    ...
                    <tbody class="bg-themeCard divide-y divide-themeBorder">
                        @forelse($brands as $brand)
                            <tr class="hover:bg-themeInput/50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-themeHeading">{{ $brand->name }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-themeBody max-w-xs truncate">
                                        {{ Str::limit($brand->description ?? '-', 50) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-themeBody">{{ $brand->products_count }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2.5 py-1 text-xs font-medium rounded-lg {{ $brand->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-themeHover text-themeBody' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-themeMuted font-medium">No brands found
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
