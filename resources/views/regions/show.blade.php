...
@section('content')
    <div class="w-full">
        <div class="mb-6 flex justify-between items-center">
            <h1 class="text-3xl font-semibold text-primary tracking-tight">{{ $region->name }}</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="text-base font-semibold text-themeHeading">{{ $region->name }}</div>
                        </div>
                        <div>
                            <div class="text-sm font-medium text-themeMuted mb-1">Status</div>
                            <span
                                class="px-3 py-1 text-sm font-medium rounded-lg {{ $region->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-themeHover text-themeBody' }}">
                        ...
                        @if ($region->description)
                            <div class="md:col-span-2">
                                ...
                                <div class="text-themeBody font-medium whitespace-pre-wrap">{{ $region->description }}</div>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                @if ($region->branches->count() > 0)
                    <div
                        class="bg-themeCard rounded-2xl border border-themeBorder p-6 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                        <h2 class="text-lg font-semibold text-primary tracking-tight mb-4">Branches
                            ({{ $region->branches->count() }})</h2>
                        <div class="space-y-3">
                            @foreach ($region->branches as $branch)
                                <div
                                    class="border border-themeBorder rounded-xl p-4 bg-themeInput/50 hover:bg-themeInput transition">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <div class="text-sm font-medium text-themeMuted mb-1">Code</div>
                                            <div class="text-base font-semibold text-themeHeading">{{ $branch->name }}</div>
                                            <div class="text-sm font-medium text-themeBody mt-1">{{ $branch->code }}</div>
                                        </div>
