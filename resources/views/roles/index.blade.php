...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-semibold text-primary tracking-tight">Roles</h1>
                <p class="text-sm font-medium text-themeMuted mt-1">Manage roles and permissions</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div
                ...
                <div class="text-sm font-medium text-themeMuted mb-1">Total Roles</div>
            ...
            <div
                ...
                <div class="text-2xl font-semibold text-primary">{{ $stats['active'] }}</div>
            </div>
            <div
                class="bg-themeCard rounded-2xl border border-themeBorder p-5 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            ...
            <div
                class="bg-themeCard rounded-2xl border border-themeBorder p-5 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                <div class="text-sm font-medium text-themeMuted mb-1">With Permissions</div>
                <div class="text-2xl font-semibold text-primary">{{ $stats['with_permissions'] }}</div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="md:hidden divide-y divide-themeBorder">
                @forelse($roles as $r)
                    <a href="{{ route('roles.show', $r) }}" class="block px-4 py-4 hover:bg-themeInput/50"><div class="text-sm font-semibold text-primary">{{ $r->name }}</div><div class="text-xs text-themeMuted">{{ $r->users_count ?? 0 }} users</div></a>
                @empty
                    <div class="px-4 py-12 text-center text-themeMuted">No roles found.</div>
                @endforelse
            </div>
                ...
                <table class="min-w-full divide-y divide-themeBorder">
                    ...
                    <tbody class="bg-themeCard divide-y divide-themeBorder">
                        @forelse($roles as $role)
                            <tr class="hover:bg-themeInput">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-themeHeading font-light font-medium">{{ $role->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-themeBody font-light">{{ $role->slug }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-themeBody font-light">
                                        {{ Str::limit($role->description ?? '-', 50) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-themeBody font-light">{{ $role->permissions_count ?? 0 }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-themeBody font-light">{{ $role->users_count }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="px-2 py-1 text-xs rounded font-light {{ $role->is_active ? 'bg-green-100 text-green-800' : 'bg-themeHover text-themeHeading' }}">
                                            {{ $role->is_active ? 'Active' : 'Inactive' }}
                                        </span>
                                        @if ($role->is_protected)
                                            <span
                                                class="px-2 py-1 text-xs rounded font-light bg-orange-100 text-orange-800">
                                                Protected
                                            </span>
                                        @endif
                                    </div>
                                        ...
                                        <div x-show="open" @click.away="open = false"
                                            ...
                                            <div class="py-1">
                                                <a href="{{ route('roles.show', $role) }}"
                                                    class="block px-4 py-2 text-sm text-themeBody hover:bg-themeHover font-light transition flex items-center space-x-2">
                                                ...
                                                @if (!$role->is_protected)
                                                    ...
                                                    @if (auth()->user()?->hasPermission('roles.delete'))
                                                        <form action="{{ route('roles.destroy', $role) }}" method="POST"
                                                            ...
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-themeHover font-light transition flex items-center space-x-2">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                                    viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                                        stroke-width="2"
                                                    ...
                                                    @endif
                                                @else
                                                    <div class="px-4 py-2 text-sm text-themeMuted font-light italic">
                                                        Protected role
                                                    </div>
                                                @endif
                                            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-4 text-center text-themeMuted font-light">No roles found.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
