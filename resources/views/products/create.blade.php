...
<div class="w-full">
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-3xl font-semibold text-primary tracking-tight">Create Product</h1>
        ...
        <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 bg-themeInput/80 border border-themeBorder rounded-xl p-4 text-sm font-medium text-themeBody">
                    Cost and selling prices are managed per region under <span class="text-themeHeading">Inventory Management → Pricing</span>. After creating the product, set its regional pricing there.
                </div>
                <div>
                    <label for="license_cost" class="block text-sm font-medium text-themeBody mb-1">License cost (cost to sell per unit)</label>
                    <input type="number" id="license_cost" name="license_cost" value="{{ old('license_cost') }}" min="0" step="0.01" placeholder="0.00"
                           class="w-full px-4 py-2.5 border border-themeBorder rounded-xl text-themeHeading focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                ...
                <div>
                    ...
                    <input type="number" id="minimum_stock_level" name="minimum_stock_level" value="{{ old('minimum_stock_level', 10) }}" min="0"
                           class="w-full px-4 py-2.5 border border-themeBorder rounded-xl text-themeHeading focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                </div>
                <div>
