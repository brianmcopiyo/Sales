...
@section('title', 'Create Field Agent')

@section('content')
    <div class="w-full space-y-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-semibold text-primary tracking-tight">Create Field Agent</h1>
                <p class="text-sm font-medium text-themeMuted mt-1">Add a new field agent (Staff user)</p>
            </div>
        ...
        <div
            ...
            <form method="POST" action="{{ route('field-agents.store') }}" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="phone" name="phone" value="{{ old('phone') }}"
                            class="w-full px-4 py-2.5 border border-themeBorder rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-medium text-themeHeading"
                            placeholder="Optional – provide email or phone">
                        <p class="text-xs font-medium text-themeMuted mt-1">At least one of email or phone is required.</p>
                        @error('phone')
                            <p class="text-xs text-red-600 font-medium mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label for="password" class="block text-sm font-medium text-themeBody mb-2">Password
                            (optional)</label>
                        <input type="password" id="password" name="password"
                            class="w-full px-4 py-2.5 border border-themeBorder rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-medium text-themeHeading">
                        <p class="text-xs font-medium text-themeMuted mt-1">Leave blank to auto-generate a temporary password.
                        </p>
                    </div>
                    <div>
                ...
                <div class="text-sm font-medium text-themeBody bg-themeInput border border-themeBorder rounded-xl p-4">
                    This will create a new user (role: <span class="font-semibold text-themeHeading">Staff</span>) in your
                    branch and attach field agent commission settings.
                </div>
