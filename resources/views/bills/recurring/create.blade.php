...
@section('title', 'New Recurring Bill Template')

@section('content')
    <div class="space-y-6">
        @include('partials.back-button', ['href' => route('bills.recurring.index'), 'label' => 'Back to Recurring Bills'])
        <div>
            <h1 class="text-3xl font-semibold text-primary tracking-tight">New Recurring Bill Template</h1>
            <p class="text-sm font-medium text-themeMuted mt-1">Set up a template to create the next bill with one click (e.g. monthly rent)</p>
        </div>
        ...
        <div class="bg-themeCard rounded-2xl border border-themeBorder p-6 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            <form method="POST" action="{{ route('bills.recurring.store') }}" class="space-y-6">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="next_due_date" name="next_due_date" value="{{ old('next_due_date') }}" required
                            class="w-full px-4 py-2.5 border border-themeBorder rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-medium text-themeHeading">
                        @error('next_due_date')
                            <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="flex gap-3 pt-2">
                    <button type="submit" class="bg-primary text-white px-5 py-2.5 rounded-xl font-medium hover:bg-primary-dark transition shadow-sm">Create template</button>
