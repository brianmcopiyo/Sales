...
@section('content')
    <div class="space-y-6">
        ...
        <div>
            <h1 class="text-3xl font-semibold text-primary tracking-tight">Edit Bill Category</h1>
            <p class="text-sm font-medium text-themeMuted mt-1">{{ $category->name }}</p>
        </div>
        ...
        <div class="bg-themeCard rounded-2xl border border-themeBorder p-6 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            <form method="POST" action="{{ route('bills.categories.update', $category) }}" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label for="name" class="block text-sm font-medium text-themeBody mb-2">Name *</label>
                        <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" required maxlength="80"
                    ...
                    <div class="md:col-span-2 flex items-center gap-2">
                        ...
                        <label for="is_active" class="text-sm font-medium text-themeBody">Active (show in bill and recurring bill forms)</label>
                    </div>
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="submit" class="bg-primary text-white px-5 py-2.5 rounded-xl font-medium hover:bg-primary-dark transition shadow-sm">Update category</button>
