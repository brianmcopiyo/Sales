...
@section('content')
    <div class="space-y-6">
        <div>
            <h1 class="text-3xl font-semibold text-primary tracking-tight">My Commissions</h1>
            <p class="text-sm font-medium text-themeMuted mt-1">Commission earned from completed sales</p>
        </div>
        ...
        <div class="bg-themeCard rounded-2xl border border-themeBorder p-6 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            <form method="GET" action="{{ route('commission-disbursements.index') }}" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    @if (request()->hasAny(['date_from', 'date_to']))
                        <a href="{{ route('commission-disbursements.index') }}" class="bg-themeHover text-themeBody px-5 py-2.5 rounded-xl font-medium hover:bg-themeBorder transition">Clear</a>
                    @endif
                </div>
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-themeBorder">
                    ...
                    <tbody class="bg-themeCard divide-y divide-themeBorder">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-themeMuted font-medium">No commission earnings yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
