    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm font-medium text-themeMuted mt-1">Manage customer records</p>
            </div>
            <div class="flex items-center gap-2">
                @if (auth()->user()?->hasPermission('customers.view'))
                    <a href="{{ route('customers.export') . (request()->query() ? '?' . http_build_query(request()->query()) : '') }}"
        ...
        @php
            $baseQuery = array_filter(['search' => request('search'), 'status' => request('status'), 'branch' => $branchFilter ?? null]);
        @endphp
        <!-- Analytics Section -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <a href="{{ route('customers.index', $baseQuery) }}"
                class="filter-card bg-themeCard rounded-2xl border border-themeBorder p-5 shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] cursor-pointer transition-all hover:shadow-lg hover:border-primary/30 {{ !request('status') && !request('date_from') ? 'ring-2 ring-primary border-primary' : '' }}"
                data-filter="customers-all">
                <div class="text-sm font-medium text-themeMuted mb-1">Total Customers</div>
                <div class="text-2xl font-semibold text-primary">{{ $stats['total'] }}</div>
            </a>
            <a href="{{ route('customers.index', array_merge($baseQuery, ['status' => 'active'])) }}"
            ...
            <a href="{{ route('customers.index', array_merge($baseQuery, ['status' => 'inactive'])) }}"
                ...
                data-filter="customers-inactive">
        ...
        <div
            class="bg-themeCard rounded-2xl border border-themeBorder shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            {{-- Mobile: list --}}
            <div class="md:hidden divide-y divide-themeBorder">
                @forelse($customers as $customer)
                    <a href="{{ auth()->user()?->hasPermission('customers.view') ? route('customers.show', $customer) : '#' }}"
                        class="block px-4 py-4 hover:bg-themeInput/50 transition-colors {{ !auth()->user()?->hasPermission('customers.view') ? 'pointer-events-none' : '' }}">
                ...
                @empty
                    <div class="px-4 py-12 text-center text-themeMuted font-medium">No customers found.</div>
                @endforelse
            </div>
            {{-- Desktop: table --}}
            <div class="hidden md:block overflow-x-auto">
                <table class="min-w-full divide-y divide-themeBorder">
                    ...
                    <tbody class="bg-themeCard divide-y divide-themeBorder" id="customers-table-body">
                        @forelse($customers as $customer)
                            <tr class="hover:bg-themeInput/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-themeHeading">{{ $customer->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-themeBody">{{ $customer->email ?? '—' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-themeBody">{{ $customer->phone ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium {{ $customer->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-themeHover text-themeBody' }}">
                                        {{ $customer->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium overflow-visible">
                                    <div class="relative inline-block text-left" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false"
                                            ...
                                            class="absolute right-0 top-full mt-1 z-50 w-48 bg-themeCard rounded-xl border border-themeBorder shadow-[0_2px_15px_-3px_rgba(0,111,120,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                                            <div class="py-1">
                                                @if (auth()->user()?->hasPermission('customers.view'))
                                                ...
                                                @if (auth()->user()?->hasPermission('customers.delete'))
                                                    <form action="{{ route('customers.delete', $customer) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-themeMuted font-medium">No customers
                                    found.</td>
                            </tr>
                        @endforelse
                    </tbody>
